...
#  Description: Recursive algorithums. 
    ...
    if start >= len(nums):
        ...
        for i in range(start + 1, len(nums)):
            if nums[i] == nums[start]:
                b += nums[i]
                count += 1
        return groupSumClump(start + count, nums, target - b) or groupSumClump(start + count, nums, target)
...
def splitArrayHelper(start, nums, sum1, sum2):
    if start >= len(nums):
        ...
        return splitArrayHelper(start+1, nums, sum1 + int(nums[start]), sum2) or splitArrayHelper(start+1, nums, sum1, sum2+int(nums[start]))
...
def splitOdd10(nums):
    b = []
    return splitOdd10Helper(nums, b, 0)


def splitOdd10Helper(a, b, i):
    if i == len(a):
        return (sum(b) % 10 == 0 and (sum(a) - sum(b)) % 2 != 0) or (sum(b) % 2 != 0 and (sum(a)-sum(b)) % 10 == 0)
    else:
        c = b[:]
        b.append(a[i])
        return splitOdd10Helper(a, b, i + 1) or splitOdd10Helper(a, c, i + 1)
...
def split53Helper(start, nums, sum1, sum2):
    if start >= len(nums):
        return sum1 == sum2
    if nums[start] % 5 == 0:
        return split53Helper(start+1, nums, sum1 + int(nums[start]), sum2)
    if nums[start] % 3 == 0:
        split53Helper(start + 1, nums, sum1, sum2 + int(nums[start]))
    return split53Helper(start + 1, nums, sum1 + int(nums[start]),sum2) or split53Helper(start + 1, nums, sum1, sum2 + int(nums[start]))
